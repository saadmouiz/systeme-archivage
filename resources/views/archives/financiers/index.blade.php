    ...
    <div class="container mx-auto px-4 py-12">
                ...
                <div class="mb-6 lg:mb-0">
                    <div class="flex items-center mb-4">
                        ...
                        <div>
                            ...
                            <p class="text-gray-600 mt-2">Gestion centralisée de vos archives financières</p>
                        </div>
                    </div>
                </div>
        ...
        <div class="mb-12">
            <div class="bg-white/80 backdrop-blur-sm rounded-3xl p-8 shadow-xl border border-white/20">
                <!-- Barre de recherche -->
                <div class="mb-8">
                    <form action="{{ route('archives.financiers.index') }}" method="GET" class="relative">
                        <div class="relative">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <div class="flex flex-wrap gap-3">
                            <a href="{{ route('archives.financiers.index') }}" 
                               class="px-6 py-3 text-sm font-medium rounded-2xl transition-all duration-300 {{ !request('type') ? 'bg-gradient-to-r from-blue-600 to-blue-700 text-white shadow-lg scale-105' : 'bg-white/60 text-gray-700 hover:bg-white hover:shadow-md border border-gray-200' }}">
                                Tous
                            </a>
                            @foreach($types as $type)
                                <a href="{{ route('archives.financiers.index', ['type' => $type]) }}" 
                                   class="px-6 py-3 text-sm font-medium rounded-2xl transition-all duration-300 {{ request('type') == $type ? 'bg-gradient-to-r from-blue-600 to-blue-700 text-white shadow-lg scale-105' : 'bg-white/60 text-gray-700 hover:bg-white hover:shadow-md border border-gray-200' }}">
                                    {{ $type }}
                                </a>
                            @endforeach
                        </div>
                    ...
                    <!-- Années -->
                    <div>
                        <h3 class="text-sm font-semibold text-gray-700 mb-4 uppercase tracking-wider">Années</h3>
                        <div class="flex flex-wrap gap-3">
                            @foreach($annees as $annee)
                                <a href="{{ route('archives.financiers.index', ['annee' => $annee]) }}" 
                                   class="px-6 py-3 text-sm font-medium rounded-2xl transition-all duration-300 {{ request('annee') == $annee ? 'bg-gradient-to-r from-blue-600 to-blue-700 text-white shadow-lg scale-105' : 'bg-white/60 text-gray-700 hover:bg-white hover:shadow-md border border-gray-200' }}">
                                    {{ $annee }}
                                </a>
                            @endforeach
                        </div>
        ...
        <div class="space-y-12">
                    ...
                    <div class="bg-white/60 backdrop-blur-sm rounded-3xl p-8 shadow-xl border border-white/20">
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8" id="docs-{{ $loop->index }}">
                            ...
                            @foreach($visibleDocs as $doc)
                                <div class="card-hover relative bg-white rounded-2xl p-6 shadow-lg hover:shadow-2xl transition-all duration-500 border border-gray-100 hover:border-blue-200 hover:-translate-y-2 cursor-pointer" 
                                     onclick="window.location.href='{{ route('archives.financiers.show', $doc['id']) }}'">
                                    <!-- Card Header -->
                                    <div class="flex items-start justify-between mb-6">
                                        <div class="flex-1">
                                            <h3 class="font-bold text-gray-900 text-lg leading-tight mb-2 hover:text-blue-600 transition-colors duration-200">{{ $doc['titre'] }}</h3>
                                            @if(isset($doc['reference']) && $doc['reference'])
                                                <p class="text-sm text-gray-500 font-mono bg-gray-50 px-3 py-1 rounded-lg inline-block hover:bg-blue-50 hover:text-blue-700 transition-all duration-200">
                                                    Réf: {{ $doc['reference'] }}
                                                </p>
                                            @endif
                                        </div>
                                        <!-- Statut à la place d'Actif -->
                                        <div class="flex items-center space-x-2">
                                            <span class="px-3 py-1 text-xs font-semibold rounded-full 
                                                {{ $doc['statut'] === 'payé' ? 'bg-gradient-to-r from-green-100 to-green-200 text-green-800 border border-green-300' : 
                                                   ($doc['statut'] === 'en attente' ? 'bg-gradient-to-r from-yellow-100 to-yellow-200 text-yellow-800 border border-yellow-300' : 
                                                   ($doc['statut'] === 'validé' ? 'bg-gradient-to-r from-blue-100 to-blue-200 text-blue-800 border border-blue-300' :
                                                   ($doc['statut'] === 'rejeté' ? 'bg-gradient-to-r from-red-100 to-red-200 text-red-800 border border-red-300' :
                                    ...
                                    <div class="space-y-4 mb-6">
                                        <div class="flex items-center justify-between p-4 bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl hover:from-blue-50 hover:to-blue-100 transition-all duration-200">
                                            <div class="flex items-center text-gray-600 hover:text-blue-700 transition-colors duration-200">
                                                <svg class="w-5 h-5 mr-3 text-gray-400 hover:text-blue-500 transition-colors duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                                                </svg>
                                                <span class="font-medium">{{ \Carbon\Carbon::parse($doc['date_document'])->format('d/m/Y') }}</span>
                                            </div>
                                            @if(isset($doc['montant']) && $doc['montant'])
                                                <div class="text-right">
                                                    <div class="text-lg font-bold text-gray-900 hover:text-blue-700 transition-colors duration-200">
                                                        {{ number_format($doc['montant'], 2, ',', ' ') }} DH
                                                    </div>
                                                </div>
                                            @endif
                                        </div>
                                    ...
                                    <div class="flex justify-between items-center pt-6 border-t border-gray-100">
                                        <div class="flex space-x-2">
                                            <a href="{{ route('archives.financiers.show', $doc['id']) }}" 
                                            ...
                                            <a href="{{ route('archives.financiers.download', $doc['id']) }}" 
                                               class="group/btn inline-flex items-center justify-center w-10 h-10 text-green-600 hover:text-white hover:bg-green-600 rounded-xl transition-all duration-300 hover:scale-110 shadow-sm hover:shadow-lg"
                                               title="Télécharger"
                                               onclick="event.stopPropagation()">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            ...
                                            <a href="{{ route('archives.financiers.edit', $doc['id']) }}" 
                                        ...
                                        <form action="{{ route('archives.financiers.destroy', $doc['id']) }}" 
                                              method="POST" 
            ...
            @empty
                <div class="text-center py-20">
                    ...
                    <p class="text-gray-600 text-lg mb-8">Commencez par ajouter votre premier document financier pour organiser vos archives.</p>
                    <a href="{{ route('archives.financiers.create') }}" 
