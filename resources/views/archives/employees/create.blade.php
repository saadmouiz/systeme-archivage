...
<div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-8">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-gray-900">Ajouter un nouvel employé</h1>
            <p class="mt-2 text-sm text-gray-600">Remplissez le formulaire ci-dessous pour ajouter un nouvel employé</p>
        </div>

        <div class="bg-white shadow-md rounded-lg p-6">
            <form action="{{ route('archives.employees.store') }}" method="POST" enctype="multipart/form-data">
                @csrf

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="nom" class="block text-sm font-medium text-gray-700">Nom</label>
                        <input type="text" name="nom" id="nom" value="{{ old('nom') }}" class="mt-1 focus:ring-red-900 focus:border-red-900 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                        @error('nom')
                    ...
                    <div>
                        ...
                        <input type="text" name="prenom" id="prenom" value="{{ old('prenom') }}" class="mt-1 focus:ring-red-900 focus:border-red-900 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                        @error('prenom')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="fonction" id="fonction" value="{{ old('fonction') }}" class="mt-1 focus:ring-red-900 focus:border-red-900 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                        @error('fonction')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" name="date_embauche" id="date_embauche" value="{{ old('date_embauche') }}" class="mt-1 focus:ring-red-900 focus:border-red-900 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                        @error('date_embauche')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" name="date_fin_contrat" id="date_fin_contrat" value="{{ old('date_fin_contrat') }}" class="mt-1 focus:ring-red-900 focus:border-red-900 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                    ...
                    <div>
                        ...
                        <div class="mt-2">
                            <div class="flex items-center">
                                <input id="actif_oui" name="actif" type="radio" value="1" class="focus:ring-red-900 h-4 w-4 text-red-900 border-gray-300" {{ old('actif', '1') == '1' ? 'checked' : '' }}>
                                <label for="actif_oui" class="ml-3 block text-sm font-medium text-gray-700">Actif</label>
                            </div>
                            <div class="flex items-center mt-2">
                                <input id="actif_non" name="actif" type="radio" value="0" class="focus:ring-red-900 h-4 w-4 text-red-900 border-gray-300" {{ old('actif') == '0' ? 'checked' : '' }}>
                ...
                <div class="mt-6">
                    ...
                    <textarea name="notes" id="notes" rows="3" class="mt-1 focus:ring-red-900 focus:border-red-900 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">{{ old('notes') }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
